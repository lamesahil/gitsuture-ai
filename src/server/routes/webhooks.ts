...
webhooksRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    // ── 3. Bot Loop Protection ────────────────────────────────────────────
    // If the event is generated by a bot (like GitSuture pushing a fix), ignore it
    // to prevent infinite feedback loops. type === 'Bot' is the stable identifier.
    if (payload.sender?.type === 'Bot') {
      console.log(`[WEBHOOK] Ignoring event from bot (sender=${payload.sender.login}) to prevent infinite loops.`);
      res.status(200).json({ message: 'Ignored bot event' });
      return;
    }
